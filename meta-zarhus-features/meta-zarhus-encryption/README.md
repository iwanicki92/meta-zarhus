...
3. Check if `<X>` device is encrypted
    - If it is then decrypt and continue with normal boot
    - If it isn't then continue to point 4
4. Shrink filesystem by 32 MB
5. Shrink partition by 32 MB
6. Encrypt partition in-place with temporary key
7. Enroll TPM2 device and wipe any other decryption method
8. Decrypt partition
9. Extend filesystem to use all available space left after encryption
10. Continue with normal boot
