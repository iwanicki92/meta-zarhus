...
class BootimgGrubLegacyPlugin(SourcePlugin):
    ...
    def do_prepare_partition(cls, part, source_params, creator, cr_workdir,
        ...
        # Copying initramfs for partition 2
        machine = get_bitbake_var("MACHINE")
        initramfs_name = f"bzImage-initramfs-{machine}.bin"
        hdddir = "%s/hdd/boot" % cr_workdir
        install_cmd = "install -m 0644 %s/%s %s/bzImage-initramfs-a" % \
            (staging_kernel_dir, initramfs_name, hdddir)
        exec_cmd(install_cmd)

        # Copying initramfs for partition 3
        hdddir = "%s/hdd/boot" % cr_workdir
        install_cmd = "install -m 0644 %s/%s  %s/bzImage-initramfs-b" % \
            (staging_kernel_dir, initramfs_name, hdddir)
        exec_cmd(install_cmd)
